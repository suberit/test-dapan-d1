#!/usr/bin/env python3
"""变体 D1: 2个并行browser实例采集板块, 1s错峰启动
...
def _scrape_block_worker(worker_idx, chunk, chunk_indices, block_results):
    ...
    print(f"  [D1] Worker {worker_idx} 启动, 处理 {len(chunk)} 个板块", flush=True)
                ...
                try:
                    ...
                    for pk in ["1day", "1hour", "7day"]:
                        ...
                        if data:
                            ...
                            print(f"    [D1-W{worker_idx}] {bname} {pk}: {len(converted)} 条", flush=True)
                    ...
                    print(f"    [D1-W{worker_idx}] 板块 {blk.get('name', '?')} 失败: {type(e).__name__}: {e}", flush=True)
            browser.close()
    except Exception as e:
        print(f"  [D1] Worker {worker_idx} 异常: {type(e).__name__}: {e}", flush=True)
...
def scrape_steamdt(page):
    """D1: 并行采集SteamDT(2 workers, 1s stagger)"""
    print(f"\n{'='*60}", flush=True)
    print(f"  抓取 SteamDT 大盘+热门板块数据 [D1 并行x2]", flush=True)
    ...
    try:
        ...
        print(f"\n[2] 并行采集 {len(block_list)} 个板块 (D1: {NUM_WORKERS} workers, {STAGGER_SECONDS}s stagger)...", flush=True)
        ...
        print(f"  [ERROR] D1采集异常: {type(e).__name__}: {e}", flush=True)
    finally:
        page.remove_listener("response", handle_response)
